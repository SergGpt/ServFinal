export default (myEventEmmiter, dispatch) => {
    ...
    myEventEmmiter.on('phone.call.story.add', (number, type, date) => {
        dispatch({
            type: 'CALL_STORY_ADD',
            payload: { number, type, date }
        });
    });
    ...
    myEventEmmiter.on('marketplace.phone.data', (lots, sellOptions, viewerCharacterId) => {
        dispatch({
            type: 'PHONE_MARKETPLACE_LOTS',
            payload: { lots, sellOptions, viewerCharacterId }
        });
    });
