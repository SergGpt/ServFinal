...
async loadInventoryItemsFromDB() {
    const dbItems = await db.Models.InventoryItem.findAll();

    this.inventoryItems = {};

    // Соберём массив уже распарсенных plain-объектов
    const parsedItems = dbItems.map(item => {
        const plainItem = item.get({ plain: true });

        // Попробуем распарсить attachInfo (если строка)
        try {
            if (plainItem.attachInfo && typeof plainItem.attachInfo === "string") {
                plainItem.attachInfo = JSON.parse(plainItem.attachInfo);
            }
        } catch (e) {
            console.error(`[INVENTORY] Ошибка парсинга attachInfo для itemId ${plainItem.id}:`, e);
            plainItem.attachInfo = null;
        }

        this.inventoryItems[plainItem.id] = plainItem;
        return plainItem;
    });

    console.log("[INVENTORY] Всего предметов загружено:", dbItems.length);
    console.log("[INVENTORY] Проверка предмета 129 (Чипсы):", this.inventoryItems[129]);
...
convertServerInventoryItemToClient(item) {
    // Парсим attachInfo из строки в объект
    let attachInfo = null;
    if (item.attachInfo) {
        try {
            attachInfo = typeof item.attachInfo === 'string' 
                ? JSON.parse(item.attachInfo) 
                : item.attachInfo;
        } catch (e) {
            console.error("Error parsing attachInfo for item", item.id, e);
            attachInfo = null;
        }
    }
    ...
    return {
        ...
        chance: item.chance,
        model: item.model,
        attachInfo: attachInfo, // Теперь это объект, а не строка
    };
},
    ...
    async initFactionCommonInventory(holder) {
        holder.inventory.items = []; // предметы

        let dbItems = await db.Models.FactionCommonInventory.findAll({
            where: {
                factionId: holder.inventory.faction.id
            },
            order: [
                ['parentId', 'ASC']
            ],
            include: [{
                model: db.Models.FactionCommonInventoryParam,
                as: "params"
            },
                {
                    model: db.Models.InventoryItem,
        ...
        console.log(`[INVENTORY] Для фракции #${holder.inventory.faction.id} загружены общие предметы (${dbItems.length} шт.)`);
        return holder;
    },
    ...
    const slot = this.findFreeSlot(player, itemId);
    if (!slot) return `Нет места для ${item.name}`;

    if (params.sex != null && params.sex != !player.character.gender) 
        return `Предмет противоположного пола`;

    const count = params.count || 1;
    const nextWeight = this.getCommonWeight(player) + (item.weight * count);

    if (nextWeight > this.maxPlayerWeight) 
        return `Превышение по весу (${nextWeight.toFixed(2)} из ${this.maxPlayerWeight} кг)`;

    return null;
},

async addItem(player, itemId, params, callback = () => {}) {
    const cantAddMsg = this.cantAdd(player, itemId, params);
    if (cantAddMsg) return callback(cantAddMsg);

    const slot = this.findFreeSlot(player, itemId);
    if (!slot) return callback(`Нет доступного слота для предмета`);

    const struct = Object.keys(params).map(key => ({ key, value: params[key] }));

    const item = db.Models.CharacterInventory.build({
        playerId: player.character.id,
        itemId: itemId,
    ...
    async addOldItem(player, item, callback = () => {}) {
        ...
        //     let weapon = this.getItemByItemId(player, item.itemId);
    ...
    async addEnvironmentItem(player, item, pocketIndex, index) {
        ...
        else if (place.type == "FactionCommon") key = "factionId";
    ...
    moveItemToPlayer(playerFrom, playerTo, item, callback = () => {}) {
        ...
        let cantAdd = this.cantAdd(playerTo, item.itemId, this.getParamsValues(item));
...
getName(itemId) {
    ...
    clearView(player, itemId) {
        ...
        var otherItems = {
            ...
            "8": () => {
                if (player.character.gender == 0) player.setClothes(4, 18, 8, 0);
    ...
    getItemWeight(player, items, weight = 0) {
        ...
        for (let i = 0; i < items.length; i++) {
            let item = items[i];
            let info = this.getInventoryItem(item.itemId);
            weight += info.weight;
            let params = this.getParamsValues(item);
            ...
            if (params.weaponHash && params.ammo) {
                let ammoId = this.getAmmoItemId(item.itemId);
                if (ammoId) weight += this.getInventoryItem(ammoId).weight * params.ammo;
            }
            let children = this.getChildren(player.inventory.items, item);
            if (children.length) {
    ...
    getCommonWeight(player) {
        let bodyItems = player.inventory.items.filter(x => !x.parentId);
    ...
    getPocketsByType(type, trunkType) {
        switch (type) {
            case "Vehicle":
                if (trunkType == 1) {
                    return [{
    ...
    getEnvironmentClientPockets(dbItems, type, trunkType) {
        var pockets = this.getPocketsByType(type, trunkType);
        for (var i = 0; i < dbItems.length; i++) {
            var dbItem = dbItems[i];
