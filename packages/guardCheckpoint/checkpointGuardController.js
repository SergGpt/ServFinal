...
class CheckpointGuardController {
    ...
    tickPost(post, now) {
        ...
        if (!target && post.state === POST_STATE.IDLE && post.targetPlayerId != null) {
    ...
    ensurePostActive(post, now) {
        ...
        post.targetStopStaySince = 0;
        post.targetOutsidePursuitSince = 0;
        post.pendingCombatCommand = null;
        post.pendingMovementCommand = null;
        post.actionSeq = Number(post.actionSeq || 0) + 1;
        post.lastTaskType = GUARD_TASK.IDLE;
        post.lastTaskData = {};
        post.lastTaskAt = now;
        this.forEachPlayersInStream(post, (player) => {
            try { player.call("guardCheckpoint:debug:state", [{ postId: post.id, active: false }]); } catch {}
        });
