...
mp.factions = {
    ...
    showMembersSelectMenu(data) {
        console.log(`showMembersSelectMenu`, data);
    ...
    registerAttachments() {
        ...
        mp.attachmentMngr.register("medicinesBox", "ex_office_swag_pills4", 11363, new mp.Vector3(0.2, 0, -0.25),
            new mp.Vector3(-15, 90, 95), {
                dict: "anim@heists@box_carry@",
                name: "idle",
                speed: 8,
                flag: 49
        ...
        mp.attachmentMngr.register("robberyBox", "prop_cash_case_01", 11363, new mp.Vector3(0.15, 0, -0.3),
            new mp.Vector3(-15, 90, 95), {
                dict: "anim@heists@box_carry@",
                name: "idle",
                speed: 8,
...
mp.events.add("factions.control.warehouse.show", (data, cat) => {
    if (mp.busy.includes('mapCase')) {
        ...
        mp.callCEFV(`selectMenu.menus['factionControlWarehouse'].init(${itemRanks}, ${clothesRanks})`);
        mp.callCEFV(`selectMenu.showByName('factionControlWarehouse')`);
    }
});
...
mp.events.add({
    ...
    mp.keys.bind(69, true, () => { // E
        ...
        // НОВОЕ: Проверяем гараж в машине
        if (local.getVariable('insideFactionGarageVehicle')) {
            if (local.vehicle) {
                mp.events.callRemote('factions.garage.park', local.vehicle.remoteId);
            }
            return;
        }
        
        // НОВОЕ: Проверяем гараж пешком
        if (local.getVariable('insideFactionGarage')) {
            mp.events.callRemote('factions.garage.menu.request');
            return;
        }
        
        // СТАРОЕ: Остальная логика (ящики)
        if (mp.factions && typeof mp.factions.boxHandler === 'function') {
            mp.factions.boxHandler();
        }
    });
},
    "factions.insideWarehouse": (inside, type) => {
        if (mp.factions && typeof mp.factions.insideWarehouse === 'function') {
            mp.factions.insideWarehouse(inside, type);
        }
    },
    
    "factions.insideFactionWarehouse": (inside) => {
        if (mp.factions && typeof mp.factions.insideFactionWarehouse === 'function') {
            mp.factions.insideFactionWarehouse(inside);
        }
    },
    
    "factions.giverank.showMenu": () => {
        if (mp.factions && typeof mp.factions.showGiveRankSelectMenu === 'function') {
            mp.factions.showGiveRankSelectMenu();
        }
    },
    
    "factions.storage.showMenu": (factionId) => {
        if (mp.factions && typeof mp.factions.showStorageSelectMenu === 'function') {
    ...
    "factions.faction.set": (val, ranks, blipsPos) => {
        if (mp.factions) {
            if (typeof mp.factions.setFaction === 'function') mp.factions.setFaction(val);
            if (typeof mp.factions.setRanks === 'function') mp.factions.setRanks(ranks);
            if (typeof mp.factions.setBlips === 'function') mp.factions.setBlips(blipsPos);
        }
    },
    
    "factions.ranks.name.set": (data) => {
        if (mp.factions && typeof mp.factions.setRankName === 'function') {
            mp.factions.setRankName(data.rank, data.name);
        }
    },
    
    "factions.control.players.show": (data) => {
        if (mp.factions && typeof mp.factions.showMembersSelectMenu === 'function') {
    ...
    "factions.control.vehicles.show": (data) => {
        ...
        } else if (mp.factions && typeof mp.factions.showVehiclesSelectMenu === 'function') {
    ...
    "factions.info.set": (info) => {
        const trySetInfo = () => {
            if (mp.factions && typeof mp.factions.setInfo === 'function') {
                mp.factions.setInfo(info);
            } else {
                setTimeout(trySetInfo, 100);
            }
        };
        trySetInfo();
    },
    ...
    "playerEnterVehicleBoot": (player, vehicle) => {
        if (mp.factions && typeof mp.factions.hasBox === 'function' && mp.factions.hasBox()) {
    ...
    // ========== НОВЫЕ СОБЫТИЯ ДЛЯ ГАРАЖА ==========
    
    "factions.garage.menu.set": (itemsJson) => {
        try {
            const items = JSON.parse(itemsJson);
            
            // Добавляем служебные кнопки
            items.push({ text: "Обновить список" });
            items.push({ text: "Закрыть меню" });
            
            // Устанавливаем items в меню через CEF
            mp.callCEFV(`selectMenu.setItems('faction_garage', ${JSON.stringify(items)})`);
            
            // Показываем меню
            mp.callCEFV(`selectMenu.showByName('faction_garage')`);
        } catch (e) {
            console.error('Ошибка меню гаража:', e);
            mp.events.call('notifications.push.error', 'Ошибка меню гаража');
        }
    },
});
...
mp.events.add('factions.garage.menu.open', () => {
    // Запрашиваем список у сервера
    mp.events.callRemote('factions.garage.menu.request');
});

mp.events.add('factions.garage.menu.set', (itemsJson) => {
    try {
        const items = JSON.parse(itemsJson);
        ...
        // Устанавливаем items в меню
        mp.callCEFV(`selectMenu.setItems('faction_garage', ${JSON.stringify(items)})`);
        
        // Показываем меню
        mp.events.call('selectMenu.show', 'faction_garage');
    } catch (e) {
        console.error('Ошибка меню гаража:', e);
        mp.events.call('notifications.push.error', 'Ошибка меню гаража');
    }
...
// клик по «Обновить список»
mp.events.add('factions.garage.menu.request', () => {
    mp.events.callRemote('factions.garage.menu.request');
});
